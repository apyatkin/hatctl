...
from ctx.config import get_config_dir, load_company_config, list_companies, save_company_config, set_nested
...
def config_add_secret(company: str, config_path: str, keychain_name: str, file_path: str | None):
    ...
    Examples:
      ...
      ctx config add-secret acme cloud.nomad.token_ref acme-nomad-token

      ctx config add-secret acme git.identity.ssh_key acme-sshkey -f ~/.ssh/key
    """
    import base64
    import subprocess
