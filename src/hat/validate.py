...
VALID_TOP_KEYS = {
    ...
    "git", "env", "ssh", "vpn", "dns", "hosts",
    "cloud", "docker", "proxy", "browser", "apps",
}

VALID_CLOUD_KEYS = {
    "aws", "kubernetes", "nomad", "vault", "consul",
    "yandex", "digitalocean", "hetzner", "terraform",
}
...
def validate_config(config: dict) -> list[ValidationError]:
    ...
    for key in config:
        if key not in VALID_TOP_KEYS:
            errors.append(ValidationError(key, "warn", f"Unknown top-level key '{key}'"))
    ...
    if "cloud" in config and isinstance(config["cloud"], dict):
        for key in config["cloud"]:
            if key not in VALID_CLOUD_KEYS:
                errors.append(ValidationError(f"cloud.{key}", "warn", f"Unknown cloud provider '{key}'"))
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if key.endswith("_ref") and isinstance(value, str):
                if ":" not in value:
                    errors.append(ValidationError(full_path, "error", f"Invalid ref format: {value}"))
                else:
                    backend = value.split(":")[0]
                    if backend not in ("keychain", "bitwarden"):
                        errors.append(ValidationError(full_path, "error", f"Unknown backend: {backend}"))
            else:
